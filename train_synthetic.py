...
def train(cfg: DictConfig):
    powerp, powere, powerl, powerm, powerme, powerscf = (
        np.zeros((10, 100)),
        np.zeros((10, 100)),
        np.zeros((10, 100)),
        np.zeros((10, 100)),
    ...
    for samples in n_list:
        ...
        for s in range(100):
            ...
            if False:
                x = torch.concat(
                    (
                        Normal(0, 1).sample(sample_shape=[3 * samples]).view(-1, 1),
                        Normal(0, 1).sample(sample_shape=[3 * samples]).view(-1, 1),
                    ),
                    1,
                )
